...
class MultiRobotCoverageEnv(gym.Env):
    ...
    def __init__(self, config: dict | None = None):
        ...
        # freezes permanently at its last position for the rest of the
        # episode and stops receiving any reward from that point on. Other
        # (still alive) robots are unaffected and keep the episode running
        # until truncation or until every robot has died.
        ...
        self.robot_alive      = np.ones(self.num_robots, dtype=bool)
        self.step_count       = 0
    ...
    def step(self, joint_actions: np.ndarray):
        ...
        permanently and earns no further reward) while every other robot
        keeps running. The episode-level `terminated` flag only fires once
        *all* robots have died; a single robot's collision never ends the
        episode for the rest of the team.
        """
        alive = self.robot_alive.copy()   # who was alive at the start of this step
        ...
        # Dead robots remain solid obstacles at their frozen position but
        # cannot themselves newly "collide" again.
        ...
        if self.terminate_on_collision:
            self.robot_alive[collided] = False

        # --- Apply movement to alive robots that did not collide ---
        for i in range(self.num_robots):
            if alive[i] and not collided[i]:
                self.robot_positions[i] = next_pos[i]
                self.robot_headings[i]  = next_hdg[i]

        self.robot_velocities[:, 0] = np.where(alive, v_cmds, 0.0)
        self.robot_velocities[:, 1] = np.where(alive, omega_cmds, 0.0)
        ...
        # Robots already dead at the start of this step get no reward at all;
        # a robot dying on this step still receives its final team_r/coll_pen.
        ...
        rewards  = np.where(alive, team_r - coll_pen - prox_pen, 0.0).astype(np.float32)

        self.step_count += 1
        truncated  = self.step_count >= self.max_steps
        terminated = bool(self.terminate_on_collision and not self.robot_alive.any())
    ...
    def _get_info(self) -> dict:
        ...
        info = {
            ...
            'robots_alive':   self.robot_alive.copy(),
            'num_robots_alive': int(self.robot_alive.sum()),
        }
        for ri, mask in enumerate(self._room_masks):
            room_total = int(mask.sum())
